...
do
	...
	if [ $EXEC -eq 1 ];then
		...
		#do
		POST="{\"funcs\" :[\"server/server.getUserEquipmentGradeByType\", \"server/server.getUserRuneSlot1GradeByType\", \"server/server.getUserRuneSlot2GradeByType\", \"server/server.getUserRuneSlot3GradeByType\" ],\"so\" : \"28250877\",\"set\" :1,\"hot\" : \"gamefix\",\"safe\" : 0,\"clean\" : 0,\"fix\" : 0}"
			#echo $POST
