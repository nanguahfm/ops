...
do
	...
	if [ $EXEC -eq 1 ];then
		...
		#do
		POST="{\"funcs\" :[\"server/server.(*Pipeline).crusadeBattleBegin\", \"server/server.GetMineBuffedSeconds\", \"server/server.GetCrusadeHeroOverrider\"],\"so\" : \"169ae2f0\",\"set\" :1,\"hot\" : \"gamefix\",\"safe\" : 0,\"clean\" : 0,\"fix\" : 0}"
			#echo $POST
