...
do
	...
	if [ $EXEC -eq 1 ];then
		...
		#do
		POST="{\"funcs\" :[\"server/server.GetCrusadeHeroOverrider\",\"server/server.(*Pipeline).crusadeBattleBegin\", \"server/server.GetMineBuffedSeconds\"],\"so\" : \"ced3494b\",\"set\" :1,\"hot\" : \"gamefix\",\"safe\" : 2,\"clean\" : 0,\"fix\" : 0}"
			#echo $POST
