...
SQL="select concat(s."name", 'SSS', c.serverid) from server_list  s, cluster_ids c where s.cluster = c."cluster" order by c.serverid"
...
do
	...
	if [ $EXEC -eq 1 ];then
		...
		#do
			...
			echo $POST
        		/usr/bin/curl -s --max-time 5 -d'{"set":1, "notme":1, "closeslave":1}' http://$PODIP:$PODPORT/debug/regionslave
		#done
	fi
    echo "<br>------------------<br>"
done
